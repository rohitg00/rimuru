...
pub use config::{CodexConfig, CodexSandboxPermissions, CodexSettings};
pub use cost::CodexCostCalculator;
pub use session::{CodexHistoryEntry, CodexSessionData, SessionParser};
...
pub struct CodexAdapter {
    ...
    cost_calculator: CodexCostCalculator,
    session_parser: SessionParser,
    subscriptions: Arc<Mutex<HashMap<Uuid, SessionEventCallback>>>,
}

impl CodexAdapter {
    pub fn new(name: &str, config: CodexConfig) -> Self {
        ...
        Self {
            ...
            cost_calculator: CodexCostCalculator::new(),
            session_parser,
    ...
    pub fn with_default_config(name: &str) -> Self {
        Self::new(name, CodexConfig::default())
    }
    ...
    fn detect_installation(&self) -> bool {
        ...
        if let Ok(output) = Command::new("which").arg("openai").output() {
            if output.status.success() {
                return true;
            }
        }
    ...
    fn has_api_key(&self) -> bool {
        env::var(&self.config.api_key_env_var).is_ok()
    }

    #[allow(dead_code)]
    fn read_settings(&self) -> Option<CodexSettings> {
        let path = self.config.config_file_path();
        if path.exists() {
            if let Ok(content) = fs::read_to_string(&path) {
                if let Ok(settings) = serde_json::from_str::<CodexSettings>(&content) {
    ...
    fn get_version(&self) -> Option<String> {
        ...
        if let Ok(output) = Command::new("openai").arg("--version").output() {
            if output.status.success() {
                let version_str = String::from_utf8_lossy(&output.stdout);
                let version = version_str.trim().to_string();
                if !version.is_empty() {
                    return Some(format!("openai-cli {}", version));
                }
            }
        }
    ...
    fn check_process_running(&self) -> bool {
        #[cfg(target_os = "macos")]
        {
            if let Ok(output) = Command::new("pgrep").arg("-f").arg("codex").output() {
        ...
        #[cfg(target_os = "windows")]
        {
                ...
                .output()
            {
                ...
                return output_str.contains("codex.exe");
            }
        }
...
impl AgentAdapter for CodexAdapter {
    ...
    async fn connect(&mut self) -> RimuruResult<()> {
        info!("Connecting to Codex adapter: {}", self.name);

        if !self.detect_installation() {
            let msg = "Codex is not installed or not configured".to_string();
        ...
        *self.error_message.lock().unwrap() = None;
        ...
        debug!("Codex adapter connected successfully");
        Ok(())
    }

    async fn disconnect(&mut self) -> RimuruResult<()> {
        info!("Disconnecting Codex adapter: {}", self.name);
    ...
    async fn health_check(&self) -> RimuruResult<bool> {
        ...
        if !installed {
            *self.status.lock().unwrap() = AdapterStatus::Error;
            *self.error_message.lock().unwrap() = Some("Codex installation not found".to_string());
            return Ok(false);
        }
...
#[async_trait]
impl CostTracker for CodexAdapter {
...
#[async_trait]
impl SessionMonitor for CodexAdapter {
...
mod tests {
    ...
    async fn test_adapter_creation() {
        let adapter = CodexAdapter::with_default_config("test-codex");

        assert_eq!(adapter.name(), "test-codex");
        assert_eq!(adapter.agent_type(), AgentType::Codex);
    }

    #[tokio::test]
    async fn test_adapter_initial_status() {
        let adapter = CodexAdapter::with_default_config("test");
    ...
    async fn test_adapter_info() {
        ...
        let config = CodexConfig::new().with_config_dir(temp_dir.path().to_path_buf());

        let adapter = CodexAdapter::new("test-adapter", config);
    ...
    async fn test_get_session_history_empty() {
        ...
        let adapter = CodexAdapter::new("test", config);
