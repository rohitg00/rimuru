...
pub use config::{OpenCodeConfig, OpenCodeSettings, ProviderConfig};
pub use cost::OpenCodeCostCalculator;
pub use session::{OpenCodeSessionData, OpenCodeState, SessionParser};
...
pub struct OpenCodeAdapter {
    ...
    cost_calculator: OpenCodeCostCalculator,
    session_parser: SessionParser,
    subscriptions: Arc<Mutex<HashMap<Uuid, SessionEventCallback>>>,
}

impl OpenCodeAdapter {
    pub fn new(name: &str, config: OpenCodeConfig) -> Self {
        let session_parser = SessionParser::new(config.data_dir.clone());
        ...
        Self {
            ...
            cost_calculator: OpenCodeCostCalculator::new(),
            session_parser,
    ...
    pub fn with_default_config(name: &str) -> Self {
        Self::new(name, OpenCodeConfig::default())
    }
    ...
    fn detect_installation(&self) -> bool {
        ...
        if let Ok(output) = Command::new("which").arg("oc").output() {
            if output.status.success() {
                return true;
            }
        }
    ...
    #[allow(dead_code)]
    fn read_settings(&self) -> Option<OpenCodeSettings> {
        let path = self.config.config_file_path();
        if path.exists() {
            if let Ok(content) = fs::read_to_string(&path) {
                if let Ok(settings) = serde_json::from_str::<OpenCodeSettings>(&content) {
    ...
    fn get_version(&self) -> Option<String> {
        ...
        if let Ok(output) = Command::new("oc").arg("--version").output() {
            if output.status.success() {
                let version_str = String::from_utf8_lossy(&output.stdout);
                let version = version_str.trim().to_string();
                if !version.is_empty() {
    ...
    fn check_process_running(&self) -> bool {
        #[cfg(target_os = "macos")]
        {
            if let Ok(output) = Command::new("pgrep").arg("-f").arg("opencode").output() {
        ...
        #[cfg(target_os = "windows")]
        {
            if let Ok(output) = Command::new("tasklist")
                .args(["/FI", "IMAGENAME eq opencode.exe"])
                .output()
            {
                let output_str = String::from_utf8_lossy(&output.stdout);
                return output_str.contains("opencode.exe");
            }
        }
    ...
    fn get_default_provider(&self) -> Option<String> {
        self.read_settings().and_then(|s| s.default_provider)
    }
}
...
impl AgentAdapter for OpenCodeAdapter {
    ...
    async fn connect(&mut self) -> RimuruResult<()> {
        info!("Connecting to OpenCode adapter: {}", self.name);

        if !self.detect_installation() {
            let msg = "OpenCode is not installed or not configured".to_string();
        ...
        *self.error_message.lock().unwrap() = None;
        ...
        debug!("OpenCode adapter connected successfully");
        Ok(())
    }

    async fn disconnect(&mut self) -> RimuruResult<()> {
        info!("Disconnecting OpenCode adapter: {}", self.name);
    ...
    async fn health_check(&self) -> RimuruResult<bool> {
        ...
        if !installed {
            ...
            *self.error_message.lock().unwrap() =
                Some("OpenCode installation not found".to_string());
            return Ok(false);
        }
...
#[async_trait]
impl CostTracker for OpenCodeAdapter {
    ...
    async fn get_model_info(&self, model_name: &str) -> RimuruResult<Option<ModelInfo>> {
        ...
        Ok(self
            .cost_calculator
            .get_model(model_name, provider.as_deref())
            .cloned())
    }
    ...
    async fn get_total_cost(&self, since: Option<DateTime<Utc>>) -> RimuruResult<f64> {
        ...
        for session in filtered_sessions {
            if let Some(cost) = session.cost_usd {
                ...
                let model = session.model_name.as_deref().unwrap_or("gpt-4o");
...
#[async_trait]
impl SessionMonitor for OpenCodeAdapter {
...
mod tests {
    ...
    async fn test_adapter_creation() {
        let adapter = OpenCodeAdapter::with_default_config("test-opencode");

        assert_eq!(adapter.name(), "test-opencode");
        assert_eq!(adapter.agent_type(), AgentType::OpenCode);
    }

    #[tokio::test]
    async fn test_adapter_initial_status() {
        let adapter = OpenCodeAdapter::with_default_config("test");
    ...
    async fn test_adapter_info() {
        ...
        let config = OpenCodeConfig::new().with_config_dir(temp_dir.path().to_path_buf());

        let adapter = OpenCodeAdapter::new("test-adapter", config);
    ...
    async fn test_get_session_history_empty() {
        ...
        let adapter = OpenCodeAdapter::new("test", config);
    ...
    #[tokio::test]
    async fn test_multi_provider_cost_calculation() {
        let adapter = OpenCodeAdapter::with_default_config("test");

        let gpt4_cost = adapter.calculate_cost(1000, 500, "gpt-4o").await.unwrap();

        let claude_cost = adapter
        ...
        assert_ne!(gpt4_cost, claude_cost);
    }
    ...
    async fn test_ollama_zero_cost() {
        ...
        let cost = adapter
            .calculate_cost(100000, 50000, "local")
            .await
            .unwrap();
