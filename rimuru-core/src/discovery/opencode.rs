...
impl OpenCodeDiscovery {
    ...
    fn check_config_file(&self, config_dir: &PathBuf) -> bool {
        ...
        config_file.exists() || config_toml.exists()
    }
    ...
    fn parse_version_from_output(&self, output: &str) -> Option<String> {
        for line in output.lines() {
            ...
            if line.to_lowercase().starts_with("opencode") {
                let parts: Vec<&str> = line.split_whitespace().collect();
                if parts.len() >= 2 {
                    let version_part = parts[1].trim_start_matches('v');
                    if version_part
                        .chars()
                        .next()
            ...
            if line
                ...
                && line.contains('.')
            {
                let version = line.split_whitespace().next().unwrap_or(line);
                return Some(version.to_string());
            }
        }
        None
...
#[async_trait]
impl AgentDiscovery for OpenCodeDiscovery {
    ...
    async fn is_installed(&self) -> bool {
        ...
        for path in self.default_config_locations() {
            if self.check_config_exists(&path) {
                debug!("Found OpenCode config at {:?}", path);
                return true;
            }
        }

        if self.find_executable().is_some() {
            debug!("Found OpenCode executable");
            return true;
        }
    ...
    async fn discover(&self) -> RimuruResult<Option<AgentInstallation>> {
        ...
        let Some(config_dir) = config_dir else {
            if check_exe_fallback {
                if let Some(exe_path) = self.find_executable() {
                    ...
                    let default_config = home.join(".opencode");

                    let version = self.get_version().await.ok().flatten();

                    let mut install =
                        AgentInstallation::new(AgentType::OpenCode, "opencode", default_config)
                    ...
                    info!("Discovered OpenCode via executable (not yet configured)");
        ...
        let data_dir_option = if data_dir.exists() {
            Some(data_dir)
        } else {
            None
        };
        ...
        let mut install =
            AgentInstallation::new(AgentType::OpenCode, "opencode", config_dir.clone())
                .with_configured(is_configured);

        if let Some(exe) = executable {
            install = install.with_executable(exe);
        }
        if let Some(data) = data_dir_option {
        ...
        info!(
            "Discovered OpenCode installation at {:?} (configured: {})",
    ...
    fn default_config_locations(&self) -> Vec<PathBuf> {
        ...
        if let Some(home) = Self::home_dir() {
            locations.push(home.join(".opencode"));
        }

        if let Ok(xdg_config) = std::env::var("XDG_CONFIG_HOME") {
            locations.push(PathBuf::from(xdg_config).join("opencode"));
        }
        ...
        if let Ok(appdata) = std::env::var("APPDATA") {
            locations.push(PathBuf::from(appdata).join("OpenCode"));
        }

        #[cfg(target_os = "windows")]
        if let Ok(localappdata) = std::env::var("LOCALAPPDATA") {
            locations.push(PathBuf::from(localappdata).join("OpenCode"));
        }
...
mod tests {
    ...
    #[test]
    fn test_opencode_discovery_new() {
        let discovery = OpenCodeDiscovery::new();
        assert_eq!(discovery.agent_type(), AgentType::OpenCode);
        assert!(discovery.custom_config_dir.is_none());
    }

    #[test]
    fn test_opencode_discovery_with_config_dir() {
        let discovery =
            OpenCodeDiscovery::new().with_config_dir(PathBuf::from("/custom/.opencode"));
    ...
    fn test_default_config_locations() {
        ...
        let has_opencode_dir = locations.iter().any(|p| {
            p.to_string_lossy().contains(".opencode") || p.to_string_lossy().contains("OpenCode")
        });
        assert!(has_opencode_dir);
    }
    ...
    async fn test_discover_with_temp_config() {
        ...
        let discovery = OpenCodeDiscovery::new().with_config_dir(config_dir.clone());

        let result = discovery.discover().await.unwrap();
        assert!(result.is_some());

        let install = result.unwrap();
        assert_eq!(install.agent_type, AgentType::OpenCode);
        assert_eq!(install.name, "opencode");
    ...
    #[tokio::test]
    async fn test_discover_with_config_toml() {
        let temp_dir = tempdir().unwrap();
        let config_dir = temp_dir.path().to_path_buf();
    ...
    async fn test_is_installed_with_temp_config() {
        ...
        let discovery = OpenCodeDiscovery::new().with_config_dir(config_dir);
    ...
    async fn test_is_not_installed() {
        let discovery =
            OpenCodeDiscovery::new().with_config_dir(PathBuf::from("/nonexistent/path/.opencode"));
    ...
    fn test_parse_version_from_output() {
        ...
        assert_eq!(
            discovery.parse_version_from_output("opencode v0.1.0"),
            Some("0.1.0".to_string())
        );

        assert_eq!(
            discovery.parse_version_from_output("OpenCode 1.2.3"),
        ...
        assert_eq!(
            discovery.parse_version_from_output("opencode\nv0.2.5\nBuild: abc123"),
    ...
    #[test]
    fn test_check_config_file_toml() {
        let temp_dir = tempdir().unwrap();
        let config_dir = temp_dir.path().to_path_buf();
        let discovery = OpenCodeDiscovery::new();
        ...
        std::fs::write(config_dir.join("config.toml"), "[section]").unwrap();
        assert!(discovery.check_config_file(&config_dir));
    }

    #[tokio::test]
    async fn test_discover_with_data_dir() {
        let temp_dir = tempdir().unwrap();
        let config_dir = temp_dir.path().to_path_buf();
        let data_dir = config_dir.join("data");
        ...
        let install = result.unwrap();
        assert_eq!(install.data_dir, Some(data_dir));
    }
}
