...
impl CodexDiscovery {
    ...
    fn check_config_file(&self, config_dir: &PathBuf) -> bool {
        ...
        let instructions_file = config_dir.join("instructions.md");
        config_file.exists() || instructions_file.exists()
    }
    ...
    fn parse_version_from_output(&self, output: &str) -> Option<String> {
        for line in output.lines() {
            ...
            if line.starts_with("codex") || line.starts_with("openai") {
...
#[async_trait]
impl AgentDiscovery for CodexDiscovery {
    ...
    async fn is_installed(&self) -> bool {
        ...
        for path in self.default_config_locations() {
            if self.check_config_exists(&path) {
                debug!("Found Codex config at {:?}", path);
                return true;
            }
        }

        if self.find_executable().is_some() {
            debug!("Found Codex executable");
            return true;
        }

        if self.has_api_key() {
            debug!("Found OPENAI_API_KEY environment variable");
            return true;
        }
    ...
    async fn discover(&self) -> RimuruResult<Option<AgentInstallation>> {
        ...
        let Some(config_dir) = config_dir else {
            if check_exe_fallback {
                if let Some(exe_path) = self.find_executable() {
                    ...
                    let default_config = home.join(".codex");

                    let version = self.get_version().await.ok().flatten();

                    let is_configured = self.has_api_key();

                    let mut install =
                        AgentInstallation::new(AgentType::Codex, "codex", default_config)
                    ...
                    info!(
                        "Discovered Codex via executable (configured via env: {})",
                        is_configured
                    );
        ...
        let is_configured = self.check_config_file(&config_dir) || self.has_api_key();
        let version = self.get_version().await.ok().flatten();
        ...
        let mut install = AgentInstallation::new(AgentType::Codex, "codex", config_dir.clone())
            .with_configured(is_configured);
        ...
        info!(
            "Discovered Codex installation at {:?} (configured: {})",
    ...
    fn default_config_locations(&self) -> Vec<PathBuf> {
        ...
        if let Some(home) = Self::home_dir() {
            locations.push(home.join(".codex"));
        }

        if let Ok(xdg_config) = std::env::var("XDG_CONFIG_HOME") {
            locations.push(PathBuf::from(xdg_config).join("codex"));
        }
        ...
        if let Ok(appdata) = std::env::var("APPDATA") {
            locations.push(PathBuf::from(appdata).join("Codex"));
        }

        #[cfg(target_os = "windows")]
        if let Ok(localappdata) = std::env::var("LOCALAPPDATA") {
            locations.push(PathBuf::from(localappdata).join("Codex"));
        }
...
mod tests {
    ...
    #[test]
    fn test_codex_discovery_new() {
        let discovery = CodexDiscovery::new();
        assert_eq!(discovery.agent_type(), AgentType::Codex);
        assert!(discovery.custom_config_dir.is_none());
    }

    #[test]
    fn test_codex_discovery_with_config_dir() {
        let discovery = CodexDiscovery::new().with_config_dir(PathBuf::from("/custom/.codex"));
    ...
    fn test_default_config_locations() {
        ...
        let has_codex_dir = locations.iter().any(|p| {
            p.to_string_lossy().contains(".codex") || p.to_string_lossy().contains("Codex")
        });
        assert!(has_codex_dir);
    }
    ...
    async fn test_discover_with_temp_config() {
        ...
        let discovery = CodexDiscovery::new().with_config_dir(config_dir.clone());

        let result = discovery.discover().await.unwrap();
        assert!(result.is_some());

        let install = result.unwrap();
        assert_eq!(install.agent_type, AgentType::Codex);
        assert_eq!(install.name, "codex");
    ...
    async fn test_is_installed_with_temp_config() {
        ...
        let discovery = CodexDiscovery::new().with_config_dir(config_dir);
    ...
    async fn test_is_not_installed() {
        let discovery =
            CodexDiscovery::new().with_config_dir(PathBuf::from("/nonexistent/path/.codex"));
    ...
    fn test_parse_version_from_output() {
        ...
        assert_eq!(
            discovery.parse_version_from_output("codex v0.5.0\nBuild: 12345"),
    ...
    #[test]
    fn test_check_instructions_file() {
        let temp_dir = tempdir().unwrap();
        let config_dir = temp_dir.path().to_path_buf();
        let discovery = CodexDiscovery::new();

        std::fs::write(config_dir.join("instructions.md"), "# Instructions").unwrap();
        assert!(discovery.check_config_file(&config_dir));
    }
}
